...
import agent_code.fml.rl as q
from agent_code.fml.features import *
from agent_code.fml.rl import Transition, EnemyTransition
...
def setup_training(self):
    ...
    self.model_stats = np.zeros((env.NUMBER_OF_ROUNDS, NUMBER_OF_FEATURES))
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.rewards[self.episode] += current_rewards
    self.rewards_acc[self.episode] += current_rewards
    self.model_stats[self.episode] = self.model
    ...
    if self.episode == env.NUMBER_OF_ROUNDS:
        ...
        with open(env.WEIGHTS_NAME, 'wb') as file:
            pickle.dump(self.model_stats, file)
